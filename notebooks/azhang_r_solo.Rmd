...
Response: the grad data distribution is left-skewed so some outliers are much lower than mean.
...
```{r}
districts %>% 
  mutate(enrollment_wt = enrollment/sum(enrollment, na.rm = TRUE)) %>% 
  summarise(weighted.mean(x= grad, w = enrollment_wt, na.rm = TRUE))
```
...
Response: Southwest/Memphis has the largest standard deviation.
